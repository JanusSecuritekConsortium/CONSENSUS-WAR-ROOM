...
Active BIOS render order is strict: the selected theme logo appears centered as its own opening stage, the console clears, and the preserved BIOS identity/header is followed by author/build/patch metadata, dense system diagnostics, a configuration table, the theme-specific subsystem array, the distinct animated loading sequence, and handoff. `generate_dense_bios_boot_lines()` is the active console/Flet layout source. The former sequential memory/device/POST generator remains available only for archived previews and compatibility regression contracts; normal startup does not use it.

Terminal boot colors are intentionally separate from GUI palette contracts. EVA/NERV uses a dark-crimson centered logo and a darker cyan-blue success state, while Arasaka uses readable light-gray data rows instead of its near-black structural secondary color. Loading bars have theme-specific geometry as well as theme-specific labels, stages, and colors.
...
BIOS headers include the active `SYSTEM_VERSION`, serial/build/theme metadata, and theme-specific authority text. Non-WH40K themes also show the real current local date. WH40K visible boot output keeps Imperial chronology only, using labels such as `DATE REF: 0918015.M03` and `CHRONO-STAMP: 0918015.M03` rather than Gregorian dates. Each active boot generation captures a fresh hardware snapshot and displays usable system RAM in MB/GiB, physical CPU cores, and logical CPU threads. Detection prefers `psutil`; if direct memory or topology detection is unavailable, the displayed fallback is marked clearly instead of being presented as measured hardware.
