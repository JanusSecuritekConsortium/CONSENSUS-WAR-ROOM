...
def format_loading_bar(theme_id: str, percent: int, ascii_only: bool = False) -> str:
    ...
    if theme_key in {"eva", "nerv"}:
        if ascii_only:
            return f"MAGI-LINK [{_fill(16, percent, '|', '.')}] {percent:3d}%"
        return f"MAGI-LINK [{_fill(16, percent, '■', '□')}] {percent:3d}%"
    if theme_key == "arasaka":
        if ascii_only:
            return f"SECURITY CLEARANCE [{_fill(10, percent, '#', '-')}] {percent:3d}%"
        return f"SECURITY CLEARANCE [{_fill(10, percent, '■', '□')}] {percent:3d}%"
    if theme_key == "janus":
        filled_char = "#" if ascii_only else "▓"
        empty_char = "-" if ascii_only else "░"
        return f"DUAL-CHANNEL SYNC [{_fill(12, percent, filled_char, empty_char)}] {percent:3d}%"
    if theme_key == "wh40k":
        if ascii_only:
...
def render_loading_console(theme_id: str, speed: str = "normal", seed: int | None = None) -> None:
    ...
    for percent in range(0, 101, 5):
        sys.stdout.write("\r" + format_loading_bar(theme_id, percent, ascii_only=ascii_only))
        sys.stdout.flush()
        time.sleep(_delay_for_step(speed, rng))
    print()
